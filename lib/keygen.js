  ...
  switch (type.toLowerCase()) {
    ...
    case 'sm2': {
      if (!curves.includes('SM2'))
        throw new Error('Unsupported key type: sm2 (SM2 curve not available)');
      return ['ec', { namedCurve: 'SM2', publicKeyEncoding, privateKeyEncoding }];
    }
    default:
...
function parseDERs(keyType, pub, priv) {
  switch (keyType) {
    ...
    case 'ec': {
      ...
      if (isSM2) {
        /*
          OpenSSH SM2 private key (no curve name field, like ed25519):
            string  "ssh-sm2"
            string  Q -- public
            string  d -- private
        */
        const keyName = Buffer.from('ssh-sm2');
        const privBuf = Buffer.allocUnsafe(
          4 + keyName.length
          + 4 + pubBin.length
          + 4 + privBin.length
