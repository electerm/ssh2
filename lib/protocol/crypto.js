...
const CIPHER_INFO = (() => {
  ...
  return {
    ...
    'sm4-ctr': info('sm4-ctr', 16, 16, 16, 0, 0, CIPHER_STREAM),
    'sm4-cbc': info('sm4-cbc', 16, 16, 0, 0, 0, 0),
...
const MAC_INFO = (() => {
  ...
  return {
    ...
    'hmac-sm3': info('sm3', 32, 32, false),
    'hmac-sm3-etm@openssh.com': info('sm3', 32, 32, true),
  };
})();
...
class ChaChaPolyCipherNative {
  ...
  encrypt(packet) {
    ...
    poly1305_auth(POLY1305_RESULT_MALLOC,
                  ...
                  payloadEnc.length,
                  polyKey);
    const mac = Buffer.from(POLY1305_RESULT_MALLOC);
    this._onWrite(mac);
...
class ChaChaPolyDecipherNative {
  ...
  decrypt(data, p, dataLen) {
    ...
    while (p < dataLen) {
      ...
      this._calcMac.set(POLY1305_RESULT_MALLOC);
...
function createCipher(config) {
  ...
  switch (outbound.cipherInfo.sslName) {
    case 'aes-128-gcm':
    case 'aes-256-gcm':
      return new AESGCMCipherNative(config);
    case 'chacha20':
      return new ChaChaPolyCipherNative(config);
    default: {
      ...
      return new GenericCipherNative(config);
    }
  }
}
...
function createDecipher(config) {
  ...
  switch (inbound.decipherInfo.sslName) {
    case 'aes-128-gcm':
    case 'aes-256-gcm':
      return new AESGCMDecipherNative(config);
    case 'chacha20':
      return new ChaChaPolyDecipherNative(config);
    default: {
      ...
      return new GenericDecipherNative(config);
    }
  }
}
...
module.exports = {
  ...
  init: (() => {
    ...
    return new Promise(async (resolve, reject) => {
      try {
        // Loader tries WASM first, falls back to pure JS automatically
        const loaded = await require('./crypto/poly1305-loader.js')();
        poly1305_auth = loaded.poly1305_auth;
        POLY1305_RESULT_MALLOC = loaded.resultBuffer;
      } catch (ex) {
        return reject(ex);
      }
      resolve();
