...
const { CIPHER_INFO, MAC_INFO } = require('./crypto.js');
...
// SM2/SM3/SM4 support detection (Chinese national cryptographic standards)
// Try native SM2 first, then fall back to sm-polyfill if available
let sm2Polyfill = null;
const sm2NativeSupported = (() => {
  const curves = crypto.getCurves();
  if (!curves.includes('SM2'))
    return false;
  // Verify that SM2 key generation and signing actually works
  try {
    const { privateKey, publicKey } = crypto.generateKeyPairSync('ec', {
      namedCurve: 'SM2',
    });
    const data = Buffer.from('sm2-support-test');
    const sig = crypto.sign('sm3', data, privateKey);
    return crypto.verify('sm3', data, publicKey, sig) === true;
  } catch {
    return false;
  }
})();

// Try to load sm-polyfill for Node.js 16 support
if (!sm2NativeSupported) {
  try {
    sm2Polyfill = require('sm-polyfill');
    if (!sm2Polyfill.isSmCryptoAvailable()) {
      sm2Polyfill = null;
    }
  } catch {
    sm2Polyfill = null;
...
// SM2 is supported if either native or polyfill is available
const sm2Supported = sm2NativeSupported || (sm2Polyfill !== null);

const sm3Supported = (crypto.getHashes().includes('sm3')
                      && typeof crypto.createHmac === 'function');

const sm4Supported = (crypto.getCiphers().includes('sm4-ctr')
                      && crypto.getCiphers().includes('sm4-cbc'));
...
if (sm2Supported && sm3Supported) {
  // SM2 Key Exchange Protocol with SM3 hash -- GM/T 0049-2016
  DEFAULT_KEX.push('sm2kep-sha3-sm3');
}
...
if (sm2Supported)
  DEFAULT_SERVER_HOST_KEY.push('ssh-sm2');
...
if (sm4Supported) {
  // SM4 (Chinese national standard block cipher) -- GM/T 0049-2016
  DEFAULT_CIPHER.push('sm4-ctr');
  DEFAULT_CIPHER.push('sm4-cbc');
}
...
if (sm3Supported) {
  // SM3 (Chinese national standard hash) -- GM/T 0049-2016
  DEFAULT_MAC.push('hmac-sm3-etm@openssh.com');
  DEFAULT_MAC.push('hmac-sm3');
}
