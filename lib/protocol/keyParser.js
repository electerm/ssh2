...
// SM2 polyfill wrapper: on Node.js <=16, sign_/verify_ fail for SM2 keys.
// We wrap them to transparently use sm-polyfill when needed.
const _origSign = sign_;
const _origVerify = verify_;
function sm2WrappedSign(algo, data, key) {
  if (algo === 'sm3' && sm2Polyfill && !sm2NativeSupported) {
    return sm2Polyfill.sign(data, key, algo);
  }
  return _origSign(algo, data, key);
}
function sm2WrappedVerify(algo, data, key, sig) {
  if (algo === 'sm3' && sm2Polyfill && !sm2NativeSupported) {
    return sm2Polyfill.verify(data, sig, key, algo);
  }
  return _origVerify(algo, data, key, sig);
}
...
const { eddsaSupported, SUPPORTED_CIPHER, sm2Polyfill, sm2NativeSupported } = require('./constants.js');
const {
  bufferSlice,
  makeBufferParser,
  readString,
...
// SM2 curve OID -- GM/T 0003-2012
const SM2_CURVE_OID = '1.2.156.10197.1.301';

function genOpenSSHSM2Pub(Q) {
  const publicKey = Buffer.allocUnsafe(4 + 7 + 4 + Q.length);

  writeUInt32BE(publicKey, 7, 0);
  publicKey.utf8Write('ssh-sm2', 4, 7);

  writeUInt32BE(publicKey, Q.length, 11);
  publicKey.set(Q, 15);
  ...
  sign: (() => {
    if (typeof sign_ === 'function') {
      return function sign(data, algo) {
        ...
        try {
          return sm2WrappedSign(algo, data, pem);
        } catch (ex) {
          return ex;
        }
      };
    ...
    if (typeof verify_ === 'function') {
      return function verify(data, signature, algo) {
        ...
        if (!algo || typeof algo !== 'string')
          algo = this[SYM_HASH_ALGO];
        try {
          return sm2WrappedVerify(algo, data, pem, signature);
        } catch (ex) {
          return ex;
        }
      };
      ...
      switch (type) {
        ...
        case 'ssh-sm2': {
          /*
            ...
            string  d -- private
          */
          const sm2pub = readString(data, data._pos);
          if (sm2pub === undefined)
            return new Error('Malformed OpenSSH private key');
          const sm2priv = readString(data, data._pos);
          if (sm2priv === undefined)
            return new Error('Malformed OpenSSH private key');

          pubPEM = genOpenSSLECDSAPub(SM2_CURVE_OID, sm2pub);
          pubSSH = genOpenSSHSM2Pub(sm2pub);
          privPEM = genOpenSSLECDSAPriv(SM2_CURVE_OID, sm2pub, sm2priv);
          algo = 'sm3';
          break;
        }
...
OpenSSH_Public.prototype = BaseKey;
{
  let regexp;
  if (eddsaSupported)
    regexp = /^(((?:ssh-(?:rsa|dss|ed25519|sm2))|ecdsa-sha2-nistp(?:256|384|521))(?:-cert-v0[01]@openssh.com)?) ([A-Z0-9a-z/+=]+)(?:$|\s+([\S].*)?)$/;
  else
    regexp = /^(((?:ssh-(?:rsa|dss|sm2))|ecdsa-sha2-nistp(?:256|384|521))(?:-cert-v0[01]@openssh.com)?) ([A-Z0-9a-z/+=]+)(?:$|\s+([\S].*)?)$/;
...
function parseDER(data, baseType, comment, fullType) {
  ...
  switch (baseType) {
    ...
    case 'ssh-sm2': {
      // SM2 public key -- GM/T 0003-2012 (no curve name field, like ed25519)
      const sm2pub = readString(data, data._pos || 0);
      if (sm2pub === undefined)
        return new Error('Malformed OpenSSH public key');
      pubPEM = genOpenSSLECDSAPub(SM2_CURVE_OID, sm2pub);
      pubSSH = genOpenSSHSM2Pub(sm2pub);
